...
class Booking
{
    ...
    {
        $available_rooms = $this->room_type->getAvailableRoomsCount($this->new_arrival_date, $this->new_departure_date);
            ...
            $hasUnavailableBooking = $room->room_bookings()
                ...
                ->where(function($query) {
                    $query->where(function($q) {
                        $q->where('arrival_date', '<=', $this->new_departure_date)
                          ->where('departure_date', '>=', $this->new_arrival_date);
                    });
                })
                ->count() > 0;
